...
# 获取脚本的目录，用于查找资源文件
script_dir = os.path.dirname(os.path.abspath(getattr(sys, 'frozen', False) and sys.executable or __file__))
...
def get_mixer_dll_paths():
    ...
    # 如果是打包后的可执行文件，也检查 PyInstaller 的临时目录
    if getattr(sys, 'frozen', False):
        base_dirs.append(sys._MEIPASS)

    try:
        import sdl2
        # 检查 PySDL2 自身的 bin 目录，那里可能包含 DLL
        sdl2_bin_path = os.path.join(os.path.dirname(sdl2.__file__), "bin")
        if os.path.exists(sdl2_bin_path):
            base_dirs.append(sdl2_bin_path)
    except ImportError:
        pass # 如果sdl2本身就没导入，跳过

...
class MyFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.audio_status_timer.Start(50) # 每 50 毫秒检查一次
    ...
    def on_toggle_monitor(self, event=None):
        ...
        if not monitoring_enabled:
            ...
            core.audio_manager.audio_command_queue.put(("stop", None)) # 确保停止播放
            stop_monitor()
    ...
    def handle_hotkey_event(self, event: HotkeyEvent):
        ...
        if func_name == "toggle_monitor":
            ...
            if not self._fast_forward_pressed:
                ...
                core.audio_manager.audio_command_queue.put(("seek", 1.0)) # 第一次立即快进
            ...
            if not self._rewind_pressed:
                ...
                core.audio_manager.audio_command_queue.put(("seek", -1.0)) # 第一次立即快退
    ...
    def on_hotkey_release_event(self, func_name):
        """处理快捷键释放事件（需要 HotkeyManager 支持）。"""
        logger.debug(f"快捷键释放: {func_name}")
        if func_name == "fast_forward":
            self._fast_forward_pressed = False
    ...
    def on_fast_forward_timer(self, event):
        ...
        if self._fast_forward_pressed:
            core.audio_manager.audio_command_queue.put(("seek", 1.0)) # 每次快进 1 秒
    ...
    def on_rewind_timer(self, event):
        ...
        if self._rewind_pressed:
            core.audio_manager.audio_command_queue.put(("seek", -1.0)) # 每次快退 1 秒
